...
class CandlePaintObject<T extends CandleIndicator>
  ...
  void paintCandleChart(Canvas canvas, Size size) {
    ...
    for (var i = start; i < end; i++) {
      ...
      Offset closeOff = Offset(dx, valueToDy(m.close));

      List<double> _yList = [highOff.dy, lowOff.dy, openOff.dy, closeOff.dy];
      _yList.sort();
      ...
      ///如果开和收价格一致，导致无法显示，显示1像素的线出来
      if (closeOff.dy == openOff.dy) {
        closeOff = Offset(closeOff.dx, openOff.dy + 1);
      }

      bool isDrawFill = true;
      if (settingConfig.barType == 1) {
        isDrawFill = false;
      } else if (settingConfig.barType == 2 && isLong) {
        isDrawFill = false;
      } else if (settingConfig.barType == 3 && !isLong) {
        isDrawFill = false;
      }
